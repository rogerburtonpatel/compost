...
We assert that the number of names and wildcards following \texttt{c}$i$ must be precisely $m$, and that all must be fresh.
...
We return the result of evaluation \texttt{e} in environment $\rho_{n+1}$.
...
The type of this expression is $\Gamma[n]$

\subsubsection{Consumption}

\texttt{n} is marked as consumed.
...
Given $\Gamma_{c}[\texttt{n}]=\tau$, we assert that $\Gamma\vdash\texttt{e}=\tau$.
\\\\
Let $v$ be the result of evaluation $\texttt{e}$ in environment $\rho$.
We bind \texttt{x} to $v$ in $\rho_{g}$ at the closing parenthesis.
\\\\
Note that if \texttt{e} produces a side effect, it is produced \textbf{only} when \texttt{n} is referenced and \textbf{every} time \texttt{x} is referenced.
