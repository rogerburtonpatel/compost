...
\subsubsection{Use Declarations}
...
\subsection{Feature 1: Memory Safety with Affine Types} {
  The killer feature of Compost is memory safety with no garbage collection.
  Importantly, we guarantee memory safety on a per-function basis.
  Any heap objects that a function allocates or is passed \textbf{will} be freed
  by that function unless they are returned from it or passed to another
  function.
  \\\\
  Our aim is to accomplish this by inserting ``free'' directives at
  \textit{compile} time.
  To do this, we need to know exactly when heap values become unreachable, and are thus fair game
  to dispose of.
  However, this is undecidable in general.
  \\\\
  To remedy this, we turn a semantic property into a syntactic one, by enforcing
  a particular way of using variables that allows us to know this at compile time.
  That is, a variable cannot be referenced if it \textbf{could} have been referenced
  by a previously evaluated expression.
  By \textbf{could}, we mean that even if it appears in only one of many branches of some previously
  evaluated expression, it is off-limits to reference.
  \\\\
  References to variables sent their values elsewhere, either returning them to the caller,
  passing them to another function, or binding them to other variables.
  With this restriction, which treats variables containing heap values as \textbf{affine-typed},
  a function can send a variable's value to only one place.
  As a result, only one reference to a value is ever live at one time, and if a variable
  goes out of scope without being referenced, its value is guaranteed never to be referenced
  again.
  \\\\
  The last property is key to our automatic insertion of ``free'' directives.
  Our algorithm traverses the AST, keeping track of those variables that become unusable
  after the current expression is evaluated.
  If we reach a terminal (a name, a function application, or a literal), any names that
  have yet to be referenced are freed, since we know that this is the point at which
  their values will never be referenced again.
}

\subsection{Feature 2: Partial Type Inference} {
  Because of time constraints, we elected not to implement full Hindley-Milner type
  inference but instead a lightweight version that requires some extra annotation.
  \\\\
  We require the programmer to annotate each function with a type.
  With this information, we begin with an initial typing environment, in which the
  names of formal parameters and all functions declared in the program are assigned
  types.
  When evaluating a given expression, the type inference function is passed this 
  environment.
  \\\\
  If the current expression binds any variables, their types will become apparent
  after recursively inferring the type of a subexpression.
  In the case of a \texttt{let} expression, inferring the type of the right-hand
  side of the binding gives the type of the bound name and in a \texttt{case} expression,
  the type of the scrutinee and the constructor named in a given branch gives the types of
  the newly bound names in that branch.
  One or more modified typing environments are created with these new bindings,
  which are then used to infer the type of the appropriate subexpresison.
  \\\\
  Any non-terminal expression's type relies on the type of one of its subexpressions;
  for example, a function application depends on the type of the called function.
  When the type inference function determines this type, it is returned as the
  result of the function.
  \\\\
  For any terminal expression, the type is either given by an axiom or the
  typing environment.
  References to variables have the type of the variable in the current typing
  environment, and each kind of literal is associated with one type.
  \\\\
  With this, we can check the types of functions with only a single
  annotation per function and infer the types of any declared local variables.
}

\subsection{Feature 3: ADTs and Top-Level Pattern Matching} {
  We support recursive abstract data types in a manner similar to ML and a
  minimally expressive pattern matching mechanism.
  \\\\
  Under the hood, datatype values are pointers to structs of 64-bit integers,
  with the first field representing the value's constructor and the remaining fields
  holding the arguments the constructor is applied to.
  \\\\
  A \texttt{case} expression compiles to a switch on the first value of the
  scrutinee struct.
  Each branch extracts the contents of each field into local variables and deallocates
  the struct.
  \\\\
  To implement abstract data types, 3 kinds of functions need to be automatically
  generated: allocation, deallocation, and deep copy.
  \\\\
  An allocation function is generated for every variant constructor
  defined for each datatype; this function simply allocates a struct on the heap, 
  stores an identifier for the constructor in the first field of the struct, and 
  stores the values of the applied arguments in the remaining fields. Any call 
  to a variant constructor in Compost maps to one of these generated allocation 
  functions.
  \\\\
  A deallocation function is generated for every datatype; this function 
  takes in a pointer to a struct representing that datatype and deallocates that 
  struct. If that struct stored any heap references (pointers to other structs), 
  those structs are also freed by calling the appropriate deallocation function. 
  Any ``free'' directive that has automatically been inserted by a previous pass 
  maps to the appropriate deallocation function depending on the datatype of the 
  instance being freed.
  \\\\
  A deep copy function is also generated for every datatype, which 
  creates a deep copy of a datatype instance. This involves copying the struct 
  pointed to by the datatype value, and for each heap reference in that struct,
  creates a deep copy of the structures pointed-to by calling the appropriate 
  deep copy function. Any ``dup'' expression in Compost maps to the appropriate 
  deep copy function depending on the datatype being copied. 
}

\subsection{Other Miscellaneous Features}

\subsubsection{Tailcall Optimization} {
  Our main code generator function, which generates code for a given expression, is
  passed a boolean telling it if the expression that it is generating code for is in tail
  position.
  As an example, if we are generating code for a \texttt{let}, the generator function
  is called on its second subexpression with this flag set to true if the \texttt{let}
  is in tail position.
  Likewise, if we are generating code for an \texttt{if} in tail position, the generator
  function is called on both of its branches with the flag set to true.
  If we are generating code for a function call in tail position, we use the LLVM API
  to mark it for a tail call optimization.
}

\subsubsection{Higher Order Functions} {
  Function values in Compost are simply function pointers, which can be used in the same
  ways as any other value except that they cannot be created inline.
  In this way, we have higher order functions but no closures, limiting their utility.
}

\subsubsection{Importing External Files} {
    We have a fairly simple yet effective method for importing external files into a Compost program.
    As part of our original parsed abstract syntax tree, a use declaration can be provided, which takes
    in a single symbol. A symbol was chosen over a name because a symbol by definition can represent any character, and
    file paths needed to support some of the characters that were reserved such as parentheses. Use
    declarations get handled as part of our preprocessor, and the post-preprocessor subset of the Ast
    (the PAst) does not include use declarations in its definitions. We use the same process as the top-level driver 
    \texttt{compost.ml} to load the
    data in the inputted file. Since a program is just a list of definitions, with use declarations being one
    of them, and a single use declaration essentially corresponds to a separate list of definitions,
    we simply expand out each ``use'' to build a final definition list. We do this recursively to handle nested use
    declarations, and the preprocessor will check to ensure that such nested declarations do not form a circular 
    reference. Additionally, since we do not allow for duplicate globals, a single file will only ever be expanded a single
    time; the preprocessor will simply ignore any subsequent use declarations for that file. For example,
    if you have library \texttt{A.gov}, and libraries \texttt{B.gov} and \texttt{C.gov} both depend on \texttt{A.gov}, 
    then both \texttt{B.gov} and \texttt{C.gov} can ``use'' \texttt{A.gov} while both being ``use''d by some other program. 
    We use the \texttt{.gov} extension in our provided library files (files meant to be ``use''d by a program) to indicate 
    that they are ``global only values,'' but also because it's funny.
}
...
\subsection{Our Approach} {
    ...
    test script along with a flag. This allowed us to run single tests without having to wait for the 
    others to finish.
    \\\\
    Our design for test files was simple. Each test is written as a \texttt{.com} file inside of our
    tests folder. Optionally, there is a file of the same name with \texttt{.in} as its extension. If
    such a file is present, then the script will automatically send the file contents in on stdin.
    Also optinally, there is a file with the same name with \texttt{.out} as its extension. This is
    to specify what the expected output of the test would be. For some tests, mostly Ast tests,
    this option is omitted, because the output is expected to be identical to the \texttt{.com} input.
    \\\\
    ...
    pretty-printing of most of the IRs when given the specified flag. Our suites are designed to 
    ...
    This is why a lot of the tests are fairly short, meant to be short and concise examples
    that adequately ensure that the behavior is what we expect. Tests in the ``run'' suite
    can be treated as integration tests because they are actually compiled down to executable 
    binaries. These test entire compost programs to ensure that
    they actually run without error. Aside from the script, we have done manual testing of
    most of these tests with \texttt{valgrind} and \texttt{leaks} to ensure memory safety of 
    our generated code.
}

\subsection{Who Did What} {
    Jackson and Roger implemented the Bash script and directory structure that composed the 
    automatic testing framework. We all contributed our own tests to the suite when implementing 
    certain features (both language features and error checking), applying the testing framework 
    to our tests to verify that they were implemented correctly. The implementer assigned to each 
    pass generally wrote a majority of the tests for that pass.
}

\subsection{Representative Source Programs}

\subsubsection{Program 1: even-odd.com}

This program tests mutual recursion between an \texttt{is-even} and \texttt{is-odd} function.
...
\begin{minted}[breaklines, frame=single]{llvm}
; ModuleID = 'Compost'
source_filename = "Compost"

@sym_lit = global [2 x i8] c"\0A\00"
...
define i1 @print-ascii(i32 %0) {
entry:
  %tmp = call i32 (i8*, ...) @printf(i8* getelementptr inbounds ([3 x i8], [3 x i8]* @fmt_char, i32 0, i32 0), i32 %0)
  ret i1 false
}
...
define i1 @print-newline() {
entry:
  %tmp = call i32 (i8*, ...) @printf(i8* getelementptr inbounds ([2 x i8], [2 x i8]* @newline, i32 0, i32 0))
  ret i1 false
}

define i1 @main() {
entry:
  %apply_result = tail call i1 @_is-even(i32 10)
  %apply_result1 = tail call i1 @print-bool(i1 %apply_result)
  %apply_result2 = tail call i1 @print-sym(i8* getelementptr inbounds ([2 x i8], [2 x i8]* @sym_lit, i32 0, i32 0))
  %apply_result3 = tail call i1 @_is-even(i32 11)
  %apply_result4 = tail call i1 @print-bool(i1 %apply_result3)
  %apply_result5 = tail call i1 @print-sym(i8* getelementptr inbounds ([2 x i8], [2 x i8]* @sym_lit, i32 0, i32 0))
  %apply_result6 = tail call i1 @_is-odd(i32 11)
  %apply_result7 = tail call i1 @print-bool(i1 %apply_result6)
  %apply_result8 = tail call i1 @print-sym(i8* getelementptr inbounds ([2 x i8], [2 x i8]* @sym_lit, i32 0, i32 0))
  %apply_result9 = tail call i1 @_is-odd(i32 12)
  %apply_result10 = tail call i1 @print-bool(i1 %apply_result9)
  %apply_result11 = tail call i1 @print-sym(i8* getelementptr inbounds ([2 x i8], [2 x i8]* @sym_lit, i32 0, i32 0))
  ret i1 %apply_result11
}

define i1 @_is-odd(i32 %0) {
entry:
  %apply_result = tail call i1 @"=i"(i32 %0, i32 1)
  br i1 %apply_result, label %then, label %else
...
then:                                             ; preds = %entry
  ...
  %apply_result1 = tail call i32 @-(i32 %0, i32 1)
  %apply_result2 = tail call i1 @_is-even(i32 %apply_result1)
  ret i1 %apply_result2
  ret i1 %apply_result2
}

define i1 @_is-even(i32 %0) {
entry:
  %apply_result = tail call i1 @"=i"(i32 %0, i32 0)
  br i1 %apply_result, label %then, label %else
...
then:                                             ; preds = %entry
  ...
  %apply_result1 = tail call i1 @_is-odd(i32 %0)
  %apply_result2 = tail call i1 @not(i1 %apply_result1)
  ret i1 %apply_result2
  ret i1 %apply_result2
}  
\end{minted}

\subsubsection{Program 2: quick\_sort.com}

This program tests the implementation of a simple quick sort function, which utilizes a list datatype, 
primitive operations, and higher-order functions.
...
@fmt_int = private unnamed_addr constant [3 x i8] c"%d\00", align 1
@newline = private unnamed_addr constant [2 x i8] c"\0A\00", align 1
@err_msg = private unnamed_addr constant [38 x i8] c"IMPOSSIBLE: inexhaustive match in dup\00", align 1
@err_msg.1 = private unnamed_addr constant [38 x i8] c"IMPOSSIBLE: inexhaustive match in dup\00", align 1
@err_msg.2 = private unnamed_addr constant [21 x i8] c"pattern match failed\00", align 1
@err_msg.3 = private unnamed_addr constant [21 x i8] c"pattern match failed\00", align 1
@err_msg.4 = private unnamed_addr constant [21 x i8] c"pattern match failed\00", align 1
@err_msg.5 = private unnamed_addr constant [21 x i8] c"pattern match failed\00", align 1
...
define i1 @_print-intlist({ i32, i64, i64 }* %0) {
entry:
  %tag_ptr = getelementptr inbounds { i32, i64, i64 }, { i32, i64, i64 }* %0, i32 0, i32 0
  %tag_val = load i32, i32* %tag_ptr, align 4
  switch i32 %tag_val, label %default [
    i32 1, label %case_branch
    i32 0, label %case_branch1
  ]

default:                                          ; preds = %entry
  %apply_result7 = tail call i1 @free_int-list({ i32, i64, i64 }* %0)
  %tmp8 = call i32 (i8*, ...) @printf(i8* getelementptr inbounds ([21 x i8], [21 x i8]* @err_msg.5, i32 0, i32 0))
  tail call void @abort()
  ret i1 true

case_branch:                                      ; preds = %entry
  %to_free = bitcast { i32, i64, i64 }* %0 to i8*
  call void @free(i8* %to_free)
  ret i1 false

case_branch1:                                     ; preds = %entry
  %arg_ptr = getelementptr inbounds { i32, i64, i64 }, { i32, i64, i64 }* %0, i32 0, i32 1
  %arg_val = load i64, i64* %arg_ptr, align 4
  %tmp = trunc i64 %arg_val to i32
  %arg_ptr2 = getelementptr inbounds { i32, i64, i64 }, { i32, i64, i64 }* %0, i32 0, i32 2
  %arg_val3 = load i64, i64* %arg_ptr2, align 4
  %tmp4 = inttoptr i64 %arg_val3 to { i32, i64, i64 }*
  %to_free5 = bitcast { i32, i64, i64 }* %0 to i8*
  call void @free(i8* %to_free5)
  %apply_result = tail call i1 @print-int(i32 %tmp)
  %apply_result6 = tail call i1 @_print-intlist({ i32, i64, i64 }* %tmp4)
  ret i1 %apply_result6
}

define { i32, i64, i64 }* @_quick-sort({ i32, i64, i64 }* %0) {
entry:
  %tag_ptr = getelementptr inbounds { i32, i64, i64 }, { i32, i64, i64 }* %0, i32 0, i32 0
  %tag_val = load i32, i32* %tag_ptr, align 4
  switch i32 %tag_val, label %default [
...
default:                                          ; preds = %entry
  %apply_result13 = tail call i1 @free_int-list({ i32, i64, i64 }* %0)
  %apply_result14 = tail call i1 @free_int-list({ i32, i64, i64 }* %0)
  %tmp15 = call i32 (i8*, ...) @printf(i8* getelementptr inbounds ([21 x i8], [21 x i8]* @err_msg.4, i32 0, i32 0))
  tail call void @abort()
  ret { i32, i64, i64 }* inttoptr (i64 -1 to { i32, i64, i64 }*)

case_branch:                                      ; preds = %entry
  %to_free = bitcast { i32, i64, i64 }* %0 to i8*
  call void @free(i8* %to_free)
  %apply_result = tail call { i32, i64, i64 }* @_nil-int-list()
  ret { i32, i64, i64 }* %apply_result
...
case_branch1:                                     ; preds = %entry
  ...
  %to_free5 = bitcast { i32, i64, i64 }* %0 to i8*
  call void @free(i8* %to_free5)
  %apply_result6 = tail call { i32, i64, i64 }* @dup_int-list({ i32, i64, i64 }* %tmp4)
  %apply_result7 = tail call { i32, i64, i64 }* @_filter-cmp(i1 (i32, i32)* @"<", i32 %tmp, { i32, i64, i64 }* %apply_result6)
  %apply_result8 = tail call { i32, i64, i64 }* @_filter-cmp(i1 (i32, i32)* @">=", i32 %tmp, { i32, i64, i64 }* %tmp4)
  %apply_result9 = tail call { i32, i64, i64 }* @_quick-sort({ i32, i64, i64 }* %apply_result7)
  %apply_result10 = tail call { i32, i64, i64 }* @_quick-sort({ i32, i64, i64 }* %apply_result8)
  %apply_result11 = tail call { i32, i64, i64 }* @_cons-int(i32 %tmp, { i32, i64, i64 }* %apply_result10)
  %apply_result12 = tail call { i32, i64, i64 }* @_concat({ i32, i64, i64 }* %apply_result9, { i32, i64, i64 }* %apply_result11)
  ret { i32, i64, i64 }* %apply_result12
}

define { i32, i64, i64 }* @_concat({ i32, i64, i64 }* %0, { i32, i64, i64 }* %1) {
entry:
  %tag_ptr = getelementptr inbounds { i32, i64, i64 }, { i32, i64, i64 }* %0, i32 0, i32 0
  %tag_val = load i32, i32* %tag_ptr, align 4
  switch i32 %tag_val, label %default [
    i32 0, label %case_branch
    i32 1, label %case_branch5
  ]

default:                                          ; preds = %entry
  %apply_result7 = tail call i1 @free_int-list({ i32, i64, i64 }* %1)
  %apply_result8 = tail call i1 @free_int-list({ i32, i64, i64 }* %0)
  %apply_result9 = tail call i1 @free_int-list({ i32, i64, i64 }* %0)
  %tmp10 = call i32 (i8*, ...) @printf(i8* getelementptr inbounds ([21 x i8], [21 x i8]* @err_msg.3, i32 0, i32 0))
  tail call void @abort()
  ret { i32, i64, i64 }* inttoptr (i64 -1 to { i32, i64, i64 }*)
...
case_branch:                                      ; preds = %entry
  ...
  %arg_val = load i64, i64* %arg_ptr, align 4
  %tmp = trunc i64 %arg_val to i32
  %arg_ptr1 = getelementptr inbounds { i32, i64, i64 }, { i32, i64, i64 }* %0, i32 0, i32 2
  %arg_val2 = load i64, i64* %arg_ptr1, align 4
  %tmp3 = inttoptr i64 %arg_val2 to { i32, i64, i64 }*
  %to_free = bitcast { i32, i64, i64 }* %0 to i8*
  call void @free(i8* %to_free)
  %apply_result = tail call { i32, i64, i64 }* @_concat({ i32, i64, i64 }* %tmp3, { i32, i64, i64 }* %1)
  %apply_result4 = tail call { i32, i64, i64 }* @_cons-int(i32 %tmp, { i32, i64, i64 }* %apply_result)
  ret { i32, i64, i64 }* %apply_result4

case_branch5:                                     ; preds = %entry
  %to_free6 = bitcast { i32, i64, i64 }* %0 to i8*
  call void @free(i8* %to_free6)
  ret { i32, i64, i64 }* %1
}

define { i32, i64, i64 }* @_filter-cmp(i1 (i32, i32)* %0, i32 %1, { i32, i64, i64 }* %2) {
entry:
  %tag_ptr = getelementptr inbounds { i32, i64, i64 }, { i32, i64, i64 }* %2, i32 0, i32 0
  %tag_val = load i32, i32* %tag_ptr, align 4
  switch i32 %tag_val, label %default [
    i32 0, label %case_branch
    i32 1, label %case_branch7
  ]

default:                                          ; preds = %entry
  %apply_result10 = tail call i1 @free_int-list({ i32, i64, i64 }* %2)
  %apply_result11 = tail call i1 @free_int-list({ i32, i64, i64 }* %2)
  %tmp12 = call i32 (i8*, ...) @printf(i8* getelementptr inbounds ([21 x i8], [21 x i8]* @err_msg.2, i32 0, i32 0))
  tail call void @abort()
  ret { i32, i64, i64 }* inttoptr (i64 -1 to { i32, i64, i64 }*)

case_branch:                                      ; preds = %entry
  %arg_ptr = getelementptr inbounds { i32, i64, i64 }, { i32, i64, i64 }* %2, i32 0, i32 1
  %arg_val = load i64, i64* %arg_ptr, align 4
  %tmp = trunc i64 %arg_val to i32
  %arg_ptr1 = getelementptr inbounds { i32, i64, i64 }, { i32, i64, i64 }* %2, i32 0, i32 2
  %arg_val2 = load i64, i64* %arg_ptr1, align 4
  %tmp3 = inttoptr i64 %arg_val2 to { i32, i64, i64 }*
  %to_free = bitcast { i32, i64, i64 }* %2 to i8*
  call void @free(i8* %to_free)
  %apply_result = tail call i1 %0(i32 %tmp, i32 %1)
  br i1 %apply_result, label %then, label %else

then:                                             ; preds = %case_branch
  %apply_result4 = tail call { i32, i64, i64 }* @_filter-cmp(i1 (i32, i32)* %0, i32 %1, { i32, i64, i64 }* %tmp3)
  %apply_result5 = tail call { i32, i64, i64 }* @_cons-int(i32 %tmp, { i32, i64, i64 }* %apply_result4)
  ret { i32, i64, i64 }* %apply_result5
  ret { i32, i64, i64 }* %apply_result5

else:                                             ; preds = %case_branch
  %apply_result6 = tail call { i32, i64, i64 }* @_filter-cmp(i1 (i32, i32)* %0, i32 %1, { i32, i64, i64 }* %tmp3)
  ret { i32, i64, i64 }* %apply_result6
  ret { i32, i64, i64 }* %apply_result6

case_branch7:                                     ; preds = %entry
  %to_free8 = bitcast { i32, i64, i64 }* %2 to i8*
  call void @free(i8* %to_free8)
  %apply_result9 = tail call { i32, i64, i64 }* @_nil-int-list()
  ret { i32, i64, i64 }* %apply_result9
}

define i1 @main() {
entry:
  %apply_result = tail call { i32, i64, i64 }* @_nil-int-list()
  %apply_result1 = tail call { i32, i64, i64 }* @_cons-int(i32 1, { i32, i64, i64 }* %apply_result)
  %apply_result2 = tail call { i32, i64, i64 }* @_cons-int(i32 3, { i32, i64, i64 }* %apply_result1)
  %apply_result3 = tail call { i32, i64, i64 }* @_cons-int(i32 5, { i32, i64, i64 }* %apply_result2)
  %apply_result4 = tail call { i32, i64, i64 }* @_cons-int(i32 2, { i32, i64, i64 }* %apply_result3)
  %apply_result5 = tail call { i32, i64, i64 }* @_cons-int(i32 4, { i32, i64, i64 }* %apply_result4)
  %apply_result6 = tail call { i32, i64, i64 }* @_quick-sort({ i32, i64, i64 }* %apply_result5)
  %apply_result7 = tail call i1 @_print-intlist({ i32, i64, i64 }* %apply_result6)
  ret i1 %apply_result7
}

define { i32, i64, i64 }* @_nil-int-list() {
entry:
  %malloccall = tail call i8* @malloc(i32 ptrtoint ({ i32, i64, i64 }* getelementptr ({ i32, i64, i64 }, { i32, i64, i64 }* null, i32 1) to i32))
  %struct = bitcast i8* %malloccall to { i32, i64, i64 }*
  %tag_ptr = getelementptr inbounds { i32, i64, i64 }, { i32, i64, i64 }* %struct, i32 0, i32 0
  store i32 1, i32* %tag_ptr, align 4
  ret { i32, i64, i64 }* %struct
}

define { i32, i64, i64 }* @_cons-int(i32 %0, { i32, i64, i64 }* %1) {
entry:
  %malloccall = tail call i8* @malloc(i32 ptrtoint ({ i32, i64, i64 }* getelementptr ({ i32, i64, i64 }, { i32, i64, i64 }* null, i32 1) to i32))
  %struct = bitcast i8* %malloccall to { i32, i64, i64 }*
  %tag_ptr = getelementptr inbounds { i32, i64, i64 }, { i32, i64, i64 }* %struct, i32 0, i32 0
  store i32 0, i32* %tag_ptr, align 4
  %tmp = zext i32 %0 to i64
  %elem_ptr = getelementptr inbounds { i32, i64, i64 }, { i32, i64, i64 }* %struct, i32 0, i32 1
  store i64 %tmp, i64* %elem_ptr, align 4
  %tmp1 = ptrtoint { i32, i64, i64 }* %1 to i64
  %elem_ptr2 = getelementptr inbounds { i32, i64, i64 }, { i32, i64, i64 }* %struct, i32 0, i32 2
  store i64 %tmp1, i64* %elem_ptr2, align 4
  ret { i32, i64, i64 }* %struct
}

define i1 @free_int-list({ i32, i64, i64 }* %0) {
entry:
  %tag_ptr = getelementptr inbounds { i32, i64, i64 }, { i32, i64, i64 }* %0, i32 0, i32 0
  %tag_val = load i32, i32* %tag_ptr, align 4
  switch i32 %tag_val, label %default [
    i32 0, label %case_branch
    i32 1, label %case_branch4
  ]

default:                                          ; preds = %entry
  %tmp6 = call i32 (i8*, ...) @printf(i8* getelementptr inbounds ([38 x i8], [38 x i8]* @err_msg.1, i32 0, i32 0))
  tail call void @abort()
  ret i1 true
...
case_branch:                                      ; preds = %entry
  ...
  %to_free = bitcast { i32, i64, i64 }* %0 to i8*
  call void @free(i8* %to_free)
  %apply_result = tail call i1 @free_int-list({ i32, i64, i64 }* %tmp3)
  ret i1 %apply_result

case_branch4:                                     ; preds = %entry
  %to_free5 = bitcast { i32, i64, i64 }* %0 to i8*
  call void @free(i8* %to_free5)
  ret i1 false
}

define { i32, i64, i64 }* @dup_int-list({ i32, i64, i64 }* %0) {
entry:
  %tag_ptr = getelementptr inbounds { i32, i64, i64 }, { i32, i64, i64 }* %0, i32 0, i32 0
  %tag_val = load i32, i32* %tag_ptr, align 4
  switch i32 %tag_val, label %default [
    i32 0, label %case_branch
    i32 1, label %case_branch8
  ]

default:                                          ; preds = %entry
  %tmp12 = call i32 (i8*, ...) @printf(i8* getelementptr inbounds ([38 x i8], [38 x i8]* @err_msg, i32 0, i32 0))
  tail call void @abort()
  ret { i32, i64, i64 }* inttoptr (i64 -1 to { i32, i64, i64 }*)
...
case_branch:                                      ; preds = %entry
  ...
  %malloccall = tail call i8* @malloc(i32 ptrtoint ({ i32, i64, i64 }* getelementptr ({ i32, i64, i64 }, { i32, i64, i64 }* null, i32 1) to i32))
  %struct = bitcast i8* %malloccall to { i32, i64, i64 }*
  %tag_ptr4 = getelementptr inbounds { i32, i64, i64 }, { i32, i64, i64 }* %struct, i32 0, i32 0
  store i32 0, i32* %tag_ptr4, align 4
  %tmp5 = zext i32 %tmp to i64
  %elem_ptr = getelementptr inbounds { i32, i64, i64 }, { i32, i64, i64 }* %struct, i32 0, i32 1
  store i64 %tmp5, i64* %elem_ptr, align 4
  %apply_result = tail call { i32, i64, i64 }* @dup_int-list({ i32, i64, i64 }* %tmp3)
  %tmp6 = ptrtoint { i32, i64, i64 }* %apply_result to i64
  %elem_ptr7 = getelementptr inbounds { i32, i64, i64 }, { i32, i64, i64 }* %struct, i32 0, i32 2
  store i64 %tmp6, i64* %elem_ptr7, align 4
  ret { i32, i64, i64 }* %struct

case_branch8:                                     ; preds = %entry
  %malloccall9 = tail call i8* @malloc(i32 ptrtoint ({ i32, i64, i64 }* getelementptr ({ i32, i64, i64 }, { i32, i64, i64 }* null, i32 1) to i32))
  %struct10 = bitcast i8* %malloccall9 to { i32, i64, i64 }*
  %tag_ptr11 = getelementptr inbounds { i32, i64, i64 }, { i32, i64, i64 }* %struct10, i32 0, i32 0
  store i32 1, i32* %tag_ptr11, align 4
  ret { i32, i64, i64 }* %struct10
}

declare noalias i8* @malloc(i32)  
\end{minted}

\subsubsection{Program 3: datatypes.com}

This program tests a variety of different datatype definitions and example use cases of those datatypes.
...
@sym_lit = global [2 x i8] c"\0A\00"
@sym_lit.1 = global [5 x i8] c"Down\00"
@sym_lit.2 = global [24 x i8] c"Finished direction test\00"
@sym_lit.3 = global [22 x i8] c"Finished intlist test\00"
@sym_lit.4 = global [24 x i8] c"Finished multilist test\00"
@sym_lit.5 = global [19 x i8] c"Finishing bst test\00"
@sym_lit.6 = global [26 x i8] c"Finishing twointlist test\00"
@sym_lit.7 = global [5 x i8] c"Left\00"
@sym_lit.8 = global [8 x i8] c"List 1:\00"
@sym_lit.9 = global [8 x i8] c"List 2:\00"
@sym_lit.10 = global [6 x i8] c"Right\00"
@sym_lit.11 = global [18 x i8] c"Starting bst test\00"
@sym_lit.12 = global [24 x i8] c"Starting direction test\00"
@sym_lit.13 = global [22 x i8] c"Starting intlist test\00"
@sym_lit.14 = global [24 x i8] c"Starting multilist test\00"
@sym_lit.15 = global [25 x i8] c"Starting twointlist test\00"
@sym_lit.16 = global [3 x i8] c"Up\00"
@unit = private unnamed_addr constant [5 x i8] c"unit\00", align 1
@true = private unnamed_addr constant [5 x i8] c"true\00", align 1
@false = private unnamed_addr constant [6 x i8] c"false\00", align 1
@fmt_char = private unnamed_addr constant [3 x i8] c"%c\00", align 1
...
@newline = private unnamed_addr constant [2 x i8] c"\0A\00", align 1
@err_msg = private unnamed_addr constant [38 x i8] c"IMPOSSIBLE: inexhaustive match in dup\00", align 1
@err_msg.17 = private unnamed_addr constant [38 x i8] c"IMPOSSIBLE: inexhaustive match in dup\00", align 1
@err_msg.18 = private unnamed_addr constant [38 x i8] c"IMPOSSIBLE: inexhaustive match in dup\00", align 1
@err_msg.19 = private unnamed_addr constant [38 x i8] c"IMPOSSIBLE: inexhaustive match in dup\00", align 1
@err_msg.20 = private unnamed_addr constant [38 x i8] c"IMPOSSIBLE: inexhaustive match in dup\00", align 1
@err_msg.21 = private unnamed_addr constant [38 x i8] c"IMPOSSIBLE: inexhaustive match in dup\00", align 1
@err_msg.22 = private unnamed_addr constant [38 x i8] c"IMPOSSIBLE: inexhaustive match in dup\00", align 1
@err_msg.23 = private unnamed_addr constant [38 x i8] c"IMPOSSIBLE: inexhaustive match in dup\00", align 1
@err_msg.24 = private unnamed_addr constant [38 x i8] c"IMPOSSIBLE: inexhaustive match in dup\00", align 1
@err_msg.25 = private unnamed_addr constant [38 x i8] c"IMPOSSIBLE: inexhaustive match in dup\00", align 1
@err_msg.26 = private unnamed_addr constant [21 x i8] c"pattern match failed\00", align 1
@err_msg.27 = private unnamed_addr constant [21 x i8] c"pattern match failed\00", align 1
@err_msg.28 = private unnamed_addr constant [21 x i8] c"pattern match failed\00", align 1
@err_msg.29 = private unnamed_addr constant [21 x i8] c"pattern match failed\00", align 1
@err_msg.30 = private unnamed_addr constant [21 x i8] c"pattern match failed\00", align 1
...
define i1 @_print-twointlist({ i32, i64, i64 }* %0) {
entry:
  %tag_ptr = getelementptr inbounds { i32, i64, i64 }, { i32, i64, i64 }* %0, i32 0, i32 0
  %tag_val = load i32, i32* %tag_ptr, align 4
  switch i32 %tag_val, label %default [
...
default:                                          ; preds = %entry
  %apply_result9 = tail call i1 @free_twointlist({ i32, i64, i64 }* %0)
  %tmp10 = call i32 (i8*, ...) @printf(i8* getelementptr inbounds ([21 x i8], [21 x i8]* @err_msg.30, i32 0, i32 0))
  tail call void @abort()
  ret i1 true
...
case_branch:                                      ; preds = %entry
  %arg_ptr = getelementptr inbounds { i32, i64, i64 }, { i32, i64, i64 }* %0, i32 0, i32 1
  %arg_val = load i64, i64* %arg_ptr, align 4
  %tmp = inttoptr i64 %arg_val to { i32, i64, i64 }*
  %arg_ptr1 = getelementptr inbounds { i32, i64, i64 }, { i32, i64, i64 }* %0, i32 0, i32 2
  %arg_val2 = load i64, i64* %arg_ptr1, align 4
  %tmp3 = inttoptr i64 %arg_val2 to { i32, i64, i64 }*
  %to_free = bitcast { i32, i64, i64 }* %0 to i8*
  call void @free(i8* %to_free)
  %apply_result = tail call i1 @print-sym(i8* getelementptr inbounds ([8 x i8], [8 x i8]* @sym_lit.8, i32 0, i32 0))
  %apply_result4 = tail call i1 @_print-endline()
  %apply_result5 = tail call i1 @_print-intlist({ i32, i64, i64 }* %tmp)
  %apply_result6 = tail call i1 @print-sym(i8* getelementptr inbounds ([8 x i8], [8 x i8]* @sym_lit.9, i32 0, i32 0))
  %apply_result7 = tail call i1 @_print-endline()
  %apply_result8 = tail call i1 @_print-intlist({ i32, i64, i64 }* %tmp3)
  ret i1 %apply_result8
}

define i1 @_test-twointlist() {
entry:
  %apply_result = tail call i1 @print-sym(i8* getelementptr inbounds ([25 x i8], [25 x i8]* @sym_lit.15, i32 0, i32 0))
  %apply_result1 = tail call i1 @_print-endline()
  %apply_result2 = tail call { i32, i64, i64 }* @_init-intlist()
  %apply_result3 = tail call { i32, i64, i64 }* @_init-intlist()
  %apply_result4 = tail call { i32, i64, i64 }* @_cons-int(i32 -1, { i32, i64, i64 }* %apply_result3)
  %apply_result5 = tail call { i32, i64, i64 }* @_build({ i32, i64, i64 }* %apply_result2, { i32, i64, i64 }* %apply_result4)
  %apply_result6 = tail call i1 @_print-twointlist({ i32, i64, i64 }* %apply_result5)
  %apply_result7 = tail call i1 @print-sym(i8* getelementptr inbounds ([26 x i8], [26 x i8]* @sym_lit.6, i32 0, i32 0))
  %apply_result8 = tail call i1 @_print-endline()
  ret i1 %apply_result8
}

define i1 @_print-multilist-ints({ i32, i64, i64 }* %0) {
entry:
  %tag_ptr = getelementptr inbounds { i32, i64, i64 }, { i32, i64, i64 }* %0, i32 0, i32 0
  %tag_val = load i32, i32* %tag_ptr, align 4
  switch i32 %tag_val, label %default [
    i32 0, label %case_branch
    i32 1, label %case_branch6
  ]

default:                                          ; preds = %entry
  %apply_result15 = tail call i1 @free_multilist({ i32, i64, i64 }* %0)
  ret i1 false
...
case_branch:                                      ; preds = %entry
  ...
  %apply_result = tail call i1 @print-int(i32 %tmp)
  %apply_result4 = tail call i1 @_print-endline()
  %apply_result5 = tail call i1 @_print-multilist-ints({ i32, i64, i64 }* %tmp3)
  ret i1 %apply_result5

case_branch6:                                     ; preds = %entry
  %arg_ptr7 = getelementptr inbounds { i32, i64, i64 }, { i32, i64, i64 }* %0, i32 0, i32 1
  %arg_val8 = load i64, i64* %arg_ptr7, align 4
  %tmp9 = trunc i64 %arg_val8 to i1
  %arg_ptr10 = getelementptr inbounds { i32, i64, i64 }, { i32, i64, i64 }* %0, i32 0, i32 2
  %arg_val11 = load i64, i64* %arg_ptr10, align 4
  %tmp12 = inttoptr i64 %arg_val11 to { i32, i64, i64 }*
  %to_free13 = bitcast { i32, i64, i64 }* %0 to i8*
  call void @free(i8* %to_free13)
  %apply_result14 = tail call i1 @_print-multilist-ints({ i32, i64, i64 }* %tmp12)
  ret i1 %apply_result14
}

define i1 @_print-multilist({ i32, i64, i64 }* %0) {
entry:
  %tag_ptr = getelementptr inbounds { i32, i64, i64 }, { i32, i64, i64 }* %0, i32 0, i32 0
  %tag_val = load i32, i32* %tag_ptr, align 4
  switch i32 %tag_val, label %default [
    i32 0, label %case_branch
    i32 1, label %case_branch6
    i32 2, label %case_branch17
  ]

default:                                          ; preds = %entry
  %apply_result19 = tail call i1 @free_multilist({ i32, i64, i64 }* %0)
  %tmp20 = call i32 (i8*, ...) @printf(i8* getelementptr inbounds ([21 x i8], [21 x i8]* @err_msg.29, i32 0, i32 0))
  tail call void @abort()
  ret i1 true
...
case_branch:                                      ; preds = %entry
  ...
  %apply_result = tail call i1 @print-int(i32 %tmp)
  %apply_result4 = tail call i1 @_print-endline()
  %apply_result5 = tail call i1 @_print-multilist({ i32, i64, i64 }* %tmp3)
  ret i1 %apply_result5
...
case_branch6:                                     ; preds = %entry
  ...
  %to_free13 = bitcast { i32, i64, i64 }* %0 to i8*
  call void @free(i8* %to_free13)
  %apply_result14 = tail call i1 @print-bool(i1 %tmp9)
  %apply_result15 = tail call i1 @_print-endline()
  %apply_result16 = tail call i1 @_print-multilist({ i32, i64, i64 }* %tmp12)
  ret i1 %apply_result16

case_branch17:                                    ; preds = %entry
  %to_free18 = bitcast { i32, i64, i64 }* %0 to i8*
  call void @free(i8* %to_free18)
  ret i1 false
}

define { i32, i64, i64 }* @_init-multilist() {
entry:
  %apply_result = tail call { i32, i64, i64 }* @_nil-multilist()
  %apply_result1 = tail call { i32, i64, i64 }* @_cons-multi-bool(i1 false, { i32, i64, i64 }* %apply_result)
  %apply_result2 = tail call { i32, i64, i64 }* @_cons-multi-int(i32 2, { i32, i64, i64 }* %apply_result1)
  %apply_result3 = tail call { i32, i64, i64 }* @_cons-multi-bool(i1 true, { i32, i64, i64 }* %apply_result2)
  %apply_result4 = tail call { i32, i64, i64 }* @_cons-multi-int(i32 1, { i32, i64, i64 }* %apply_result3)
  ret { i32, i64, i64 }* %apply_result4
}

define i1 @_test-multilist() {
entry:
  %apply_result = tail call i1 @print-sym(i8* getelementptr inbounds ([24 x i8], [24 x i8]* @sym_lit.14, i32 0, i32 0))
  %apply_result1 = tail call i1 @_print-endline()
  %apply_result2 = tail call { i32, i64, i64 }* @_init-multilist()
  %apply_result3 = tail call { i32, i64, i64 }* @dup_multilist({ i32, i64, i64 }* %apply_result2)
  %apply_result4 = tail call i1 @_print-multilist({ i32, i64, i64 }* %apply_result2)
  %apply_result5 = tail call i1 @_print-multilist-ints({ i32, i64, i64 }* %apply_result3)
  %apply_result6 = tail call i1 @print-sym(i8* getelementptr inbounds ([24 x i8], [24 x i8]* @sym_lit.4, i32 0, i32 0))
  %apply_result7 = tail call i1 @_print-endline()
  ret i1 %apply_result7
}

define i1 @_in-order-print({ i32, i64, i64, i64 }* %0) {
entry:
  %tag_ptr = getelementptr inbounds { i32, i64, i64, i64 }, { i32, i64, i64, i64 }* %0, i32 0, i32 0
  %tag_val = load i32, i32* %tag_ptr, align 4
  switch i32 %tag_val, label %default [
    i32 1, label %case_branch
    i32 0, label %case_branch1
...
default:                                          ; preds = %entry
  %apply_result11 = tail call i1 @free_bst({ i32, i64, i64, i64 }* %0)
  %tmp12 = call i32 (i8*, ...) @printf(i8* getelementptr inbounds ([21 x i8], [21 x i8]* @err_msg.28, i32 0, i32 0))
  tail call void @abort()
  ret i1 true

case_branch:                                      ; preds = %entry
  %to_free = bitcast { i32, i64, i64, i64 }* %0 to i8*
  call void @free(i8* %to_free)
  ret i1 false

case_branch1:                                     ; preds = %entry
  %arg_ptr = getelementptr inbounds { i32, i64, i64, i64 }, { i32, i64, i64, i64 }* %0, i32 0, i32 1
  %arg_val = load i64, i64* %arg_ptr, align 4
  %tmp = trunc i64 %arg_val to i32
  %arg_ptr2 = getelementptr inbounds { i32, i64, i64, i64 }, { i32, i64, i64, i64 }* %0, i32 0, i32 2
  %arg_val3 = load i64, i64* %arg_ptr2, align 4
  %tmp4 = inttoptr i64 %arg_val3 to { i32, i64, i64, i64 }*
  %arg_ptr5 = getelementptr inbounds { i32, i64, i64, i64 }, { i32, i64, i64, i64 }* %0, i32 0, i32 3
  %arg_val6 = load i64, i64* %arg_ptr5, align 4
  %tmp7 = inttoptr i64 %arg_val6 to { i32, i64, i64, i64 }*
  %to_free8 = bitcast { i32, i64, i64, i64 }* %0 to i8*
  call void @free(i8* %to_free8)
  %apply_result = tail call i1 @_in-order-print({ i32, i64, i64, i64 }* %tmp4)
  %apply_result9 = tail call i1 @print-int(i32 %tmp)
  %apply_result10 = tail call i1 @_in-order-print({ i32, i64, i64, i64 }* %tmp7)
  ret i1 %apply_result10
}

define { i32, i64, i64, i64 }* @_init-bst() {
entry:
  %apply_result = tail call { i32, i64, i64, i64 }* @_nil-bst()
  %apply_result1 = tail call { i32, i64, i64, i64 }* @_nil-bst()
  %apply_result2 = tail call { i32, i64, i64, i64 }* @_buildbst(i32 0, { i32, i64, i64, i64 }* %apply_result, { i32, i64, i64, i64 }* %apply_result1)
  %apply_result3 = tail call { i32, i64, i64, i64 }* @_nil-bst()
  %apply_result4 = tail call { i32, i64, i64, i64 }* @_nil-bst()
  %apply_result5 = tail call { i32, i64, i64, i64 }* @_buildbst(i32 2, { i32, i64, i64, i64 }* %apply_result3, { i32, i64, i64, i64 }* %apply_result4)
  %apply_result6 = tail call { i32, i64, i64, i64 }* @_nil-bst()
  %apply_result7 = tail call { i32, i64, i64, i64 }* @_nil-bst()
  %apply_result8 = tail call { i32, i64, i64, i64 }* @_buildbst(i32 4, { i32, i64, i64, i64 }* %apply_result6, { i32, i64, i64, i64 }* %apply_result7)
  %apply_result9 = tail call { i32, i64, i64, i64 }* @_buildbst(i32 1, { i32, i64, i64, i64 }* %apply_result2, { i32, i64, i64, i64 }* %apply_result5)
  %apply_result10 = tail call { i32, i64, i64, i64 }* @_nil-bst()
  %apply_result11 = tail call { i32, i64, i64, i64 }* @_buildbst(i32 5, { i32, i64, i64, i64 }* %apply_result8, { i32, i64, i64, i64 }* %apply_result10)
  %apply_result12 = tail call { i32, i64, i64, i64 }* @_buildbst(i32 3, { i32, i64, i64, i64 }* %apply_result9, { i32, i64, i64, i64 }* %apply_result11)
  ret { i32, i64, i64, i64 }* %apply_result12
}

define i1 @_test-bst() {
entry:
  %apply_result = tail call i1 @print-sym(i8* getelementptr inbounds ([18 x i8], [18 x i8]* @sym_lit.11, i32 0, i32 0))
  %apply_result1 = tail call i1 @_print-endline()
  %apply_result2 = tail call { i32, i64, i64, i64 }* @_init-bst()
  %apply_result3 = tail call { i32, i64, i64, i64 }* @dup_bst({ i32, i64, i64, i64 }* %apply_result2)
  %apply_result4 = tail call i1 @free_bst({ i32, i64, i64, i64 }* %apply_result2)
  %apply_result5 = tail call i1 @_in-order-print({ i32, i64, i64, i64 }* %apply_result3)
  %apply_result6 = tail call i1 @_print-endline()
  %apply_result7 = tail call i1 @print-sym(i8* getelementptr inbounds ([19 x i8], [19 x i8]* @sym_lit.5, i32 0, i32 0))
  %apply_result8 = tail call i1 @_print-endline()
  ret i1 %apply_result8
}

define i1 @_print-dir({ i32 }* %0) {
entry:
  %tag_ptr = getelementptr inbounds { i32 }, { i32 }* %0, i32 0, i32 0
  %tag_val = load i32, i32* %tag_ptr, align 4
  switch i32 %tag_val, label %default [
    i32 0, label %case_branch
    i32 1, label %case_branch1
    i32 2, label %case_branch4
    i32 3, label %case_branch7
  ]

default:                                          ; preds = %entry
  %apply_result10 = tail call i1 @free_direction({ i32 }* %0)
  %tmp = call i32 (i8*, ...) @printf(i8* getelementptr inbounds ([21 x i8], [21 x i8]* @err_msg.27, i32 0, i32 0))
  tail call void @abort()
  br label %merge

merge:                                            ; preds = %default, %case_branch7, %case_branch4, %case_branch1, %case_branch
  %case_result = phi i1 [ %apply_result, %case_branch ], [ %apply_result3, %case_branch1 ], [ %apply_result6, %case_branch4 ], [ %apply_result9, %case_branch7 ], [ true, %default ]
  %apply_result11 = tail call i1 @_print-endline()
  ret i1 %apply_result11

case_branch:                                      ; preds = %entry
  %to_free = bitcast { i32 }* %0 to i8*
  call void @free(i8* %to_free)
  %apply_result = tail call i1 @print-sym(i8* getelementptr inbounds ([5 x i8], [5 x i8]* @sym_lit.7, i32 0, i32 0))
  br label %merge

case_branch1:                                     ; preds = %entry
  %to_free2 = bitcast { i32 }* %0 to i8*
  call void @free(i8* %to_free2)
  %apply_result3 = tail call i1 @print-sym(i8* getelementptr inbounds ([6 x i8], [6 x i8]* @sym_lit.10, i32 0, i32 0))
  br label %merge

case_branch4:                                     ; preds = %entry
  %to_free5 = bitcast { i32 }* %0 to i8*
  call void @free(i8* %to_free5)
  %apply_result6 = tail call i1 @print-sym(i8* getelementptr inbounds ([3 x i8], [3 x i8]* @sym_lit.16, i32 0, i32 0))
  br label %merge

case_branch7:                                     ; preds = %entry
  %to_free8 = bitcast { i32 }* %0 to i8*
  call void @free(i8* %to_free8)
  %apply_result9 = tail call i1 @print-sym(i8* getelementptr inbounds ([5 x i8], [5 x i8]* @sym_lit.1, i32 0, i32 0))
  br label %merge
}

define i1 @_test-direction() {
entry:
  %apply_result = tail call i1 @print-sym(i8* getelementptr inbounds ([24 x i8], [24 x i8]* @sym_lit.12, i32 0, i32 0))
  %apply_result1 = tail call i1 @_print-endline()
  %apply_result2 = tail call { i32 }* @_left()
  %apply_result3 = tail call { i32 }* @_right()
  %apply_result4 = tail call { i32 }* @dup_direction({ i32 }* %apply_result2)
  %apply_result5 = tail call i1 @_print-dir({ i32 }* %apply_result4)
  %apply_result6 = tail call i1 @free_direction({ i32 }* %apply_result2)
  %apply_result7 = tail call i1 @_print-dir({ i32 }* %apply_result3)
  %apply_result8 = tail call i1 @print-sym(i8* getelementptr inbounds ([24 x i8], [24 x i8]* @sym_lit.2, i32 0, i32 0))
  %apply_result9 = tail call i1 @_print-endline()
  ret i1 %apply_result9
}
...
default:                                          ; preds = %entry
  %apply_result8 = tail call i1 @free_intlist({ i32, i64, i64 }* %0)
  %tmp9 = call i32 (i8*, ...) @printf(i8* getelementptr inbounds ([21 x i8], [21 x i8]* @err_msg.26, i32 0, i32 0))
  tail call void @abort()
  ret i1 true
...
case_branch:                                      ; preds = %entry
  ...
  call void @free(i8* %to_free)
  %apply_result = tail call i1 @print-int(i32 %tmp)
  %apply_result4 = tail call i1 @_print-intlist({ i32, i64, i64 }* %tmp3)
  ret i1 %apply_result4
...
define { i32, i64, i64 }* @_init-intlist() {
entry:
  %apply_result = tail call { i32, i64, i64 }* @_nil-intlist()
  %apply_result1 = tail call { i32, i64, i64 }* @_cons-int(i32 2, { i32, i64, i64 }* %apply_result)
  %apply_result2 = tail call { i32, i64, i64 }* @_cons-int(i32 1, { i32, i64, i64 }* %apply_result1)
  %apply_result3 = tail call { i32, i64, i64 }* @_cons-int(i32 0, { i32, i64, i64 }* %apply_result2)
  ret { i32, i64, i64 }* %apply_result3
}

define i1 @_test-intlist() {
entry:
  %apply_result = tail call i1 @print-sym(i8* getelementptr inbounds ([22 x i8], [22 x i8]* @sym_lit.13, i32 0, i32 0))
  %apply_result1 = tail call i1 @_print-endline()
  %apply_result2 = tail call { i32, i64, i64 }* @_init-intlist()
  %apply_result3 = tail call { i32, i64, i64 }* @dup_intlist({ i32, i64, i64 }* %apply_result2)
  %apply_result4 = tail call i1 @free_intlist({ i32, i64, i64 }* %apply_result2)
  %apply_result5 = tail call i1 @_print-intlist({ i32, i64, i64 }* %apply_result3)
  %apply_result6 = tail call i1 @print-sym(i8* getelementptr inbounds ([22 x i8], [22 x i8]* @sym_lit.3, i32 0, i32 0))
  %apply_result7 = tail call i1 @_print-endline()
  ret i1 %apply_result7
}
...
define i1 @main() {
entry:
  %apply_result = tail call i1 @_test-intlist()
  %apply_result1 = tail call i1 @_test-direction()
  %apply_result2 = tail call i1 @_test-bst()
  %apply_result3 = tail call i1 @_test-multilist()
  %apply_result4 = tail call i1 @_test-twointlist()
  ret i1 %apply_result4
}

define i1 @_print-endline() {
entry:
  %apply_result = tail call i1 @print-sym(i8* getelementptr inbounds ([2 x i8], [2 x i8]* @sym_lit, i32 0, i32 0))
  ret i1 %apply_result
}

define { i32, i64, i64 }* @_build({ i32, i64, i64 }* %0, { i32, i64, i64 }* %1) {
entry:
  %malloccall = tail call i8* @malloc(i32 ptrtoint ({ i32, i64, i64 }* getelementptr ({ i32, i64, i64 }, { i32, i64, i64 }* null, i32 1) to i32))
  %struct = bitcast i8* %malloccall to { i32, i64, i64 }*
  %tag_ptr = getelementptr inbounds { i32, i64, i64 }, { i32, i64, i64 }* %struct, i32 0, i32 0
  store i32 0, i32* %tag_ptr, align 4
  %tmp = ptrtoint { i32, i64, i64 }* %0 to i64
  %elem_ptr = getelementptr inbounds { i32, i64, i64 }, { i32, i64, i64 }* %struct, i32 0, i32 1
  store i64 %tmp, i64* %elem_ptr, align 4
  %tmp1 = ptrtoint { i32, i64, i64 }* %1 to i64
  %elem_ptr2 = getelementptr inbounds { i32, i64, i64 }, { i32, i64, i64 }* %struct, i32 0, i32 2
...
define i1 @free_twointlist({ i32, i64, i64 }* %0) {
entry:
  %tag_ptr = getelementptr inbounds { i32, i64, i64 }, { i32, i64, i64 }* %0, i32 0, i32 0
  %tag_val = load i32, i32* %tag_ptr, align 4
  switch i32 %tag_val, label %default [
...
default:                                          ; preds = %entry
  %tmp5 = call i32 (i8*, ...) @printf(i8* getelementptr inbounds ([38 x i8], [38 x i8]* @err_msg.25, i32 0, i32 0))
  tail call void @abort()
  ret i1 true
...
case_branch:                                      ; preds = %entry
  ...
  %to_free = bitcast { i32, i64, i64 }* %0 to i8*
  call void @free(i8* %to_free)
  %apply_result = tail call i1 @free_intlist({ i32, i64, i64 }* %tmp)
  %apply_result4 = tail call i1 @free_intlist({ i32, i64, i64 }* %tmp3)
  ret i1 %apply_result4
}

define { i32, i64, i64 }* @dup_twointlist({ i32, i64, i64 }* %0) {
entry:
  %tag_ptr = getelementptr inbounds { i32, i64, i64 }, { i32, i64, i64 }* %0, i32 0, i32 0
  %tag_val = load i32, i32* %tag_ptr, align 4
  switch i32 %tag_val, label %default [
...
default:                                          ; preds = %entry
  %tmp9 = call i32 (i8*, ...) @printf(i8* getelementptr inbounds ([38 x i8], [38 x i8]* @err_msg.24, i32 0, i32 0))
  tail call void @abort()
  ret { i32, i64, i64 }* inttoptr (i64 -1 to { i32, i64, i64 }*)
...
case_branch:                                      ; preds = %entry
  ...
  %tag_ptr4 = getelementptr inbounds { i32, i64, i64 }, { i32, i64, i64 }* %struct, i32 0, i32 0
  store i32 0, i32* %tag_ptr4, align 4
  %apply_result = tail call { i32, i64, i64 }* @dup_intlist({ i32, i64, i64 }* %tmp)
  %tmp5 = ptrtoint { i32, i64, i64 }* %apply_result to i64
  %elem_ptr = getelementptr inbounds { i32, i64, i64 }, { i32, i64, i64 }* %struct, i32 0, i32 1
  store i64 %tmp5, i64* %elem_ptr, align 4
  %apply_result6 = tail call { i32, i64, i64 }* @dup_intlist({ i32, i64, i64 }* %tmp3)
  %tmp7 = ptrtoint { i32, i64, i64 }* %apply_result6 to i64
  %elem_ptr8 = getelementptr inbounds { i32, i64, i64 }, { i32, i64, i64 }* %struct, i32 0, i32 2
  store i64 %tmp7, i64* %elem_ptr8, align 4
  ret { i32, i64, i64 }* %struct
}

define { i32, i64, i64 }* @_nil-multilist() {
entry:
  %malloccall = tail call i8* @malloc(i32 ptrtoint ({ i32, i64, i64 }* getelementptr ({ i32, i64, i64 }, { i32, i64, i64 }* null, i32 1) to i32))
  %struct = bitcast i8* %malloccall to { i32, i64, i64 }*
  %tag_ptr = getelementptr inbounds { i32, i64, i64 }, { i32, i64, i64 }* %struct, i32 0, i32 0
  store i32 2, i32* %tag_ptr, align 4
  ret { i32, i64, i64 }* %struct
}

define { i32, i64, i64 }* @_cons-multi-bool(i1 %0, { i32, i64, i64 }* %1) {
entry:
  %malloccall = tail call i8* @malloc(i32 ptrtoint ({ i32, i64, i64 }* getelementptr ({ i32, i64, i64 }, { i32, i64, i64 }* null, i32 1) to i32))
  %struct = bitcast i8* %malloccall to { i32, i64, i64 }*
  %tag_ptr = getelementptr inbounds { i32, i64, i64 }, { i32, i64, i64 }* %struct, i32 0, i32 0
  store i32 1, i32* %tag_ptr, align 4
  %tmp = zext i1 %0 to i64
  %elem_ptr = getelementptr inbounds { i32, i64, i64 }, { i32, i64, i64 }* %struct, i32 0, i32 1
  store i64 %tmp, i64* %elem_ptr, align 4
  %tmp1 = ptrtoint { i32, i64, i64 }* %1 to i64
  %elem_ptr2 = getelementptr inbounds { i32, i64, i64 }, { i32, i64, i64 }* %struct, i32 0, i32 2
...
define { i32, i64, i64 }* @_cons-multi-int(i32 %0, { i32, i64, i64 }* %1) {
entry:
  %malloccall = tail call i8* @malloc(i32 ptrtoint ({ i32, i64, i64 }* getelementptr ({ i32, i64, i64 }, { i32, i64, i64 }* null, i32 1) to i32))
  %struct = bitcast i8* %malloccall to { i32, i64, i64 }*
  %tag_ptr = getelementptr inbounds { i32, i64, i64 }, { i32, i64, i64 }* %struct, i32 0, i32 0
...
define i1 @free_multilist({ i32, i64, i64 }* %0) {
entry:
  %tag_ptr = getelementptr inbounds { i32, i64, i64 }, { i32, i64, i64 }* %0, i32 0, i32 0
  %tag_val = load i32, i32* %tag_ptr, align 4
  switch i32 %tag_val, label %default [
    i32 0, label %case_branch
    i32 1, label %case_branch4
    i32 2, label %case_branch13
  ]

default:                                          ; preds = %entry
  %tmp15 = call i32 (i8*, ...) @printf(i8* getelementptr inbounds ([38 x i8], [38 x i8]* @err_msg.23, i32 0, i32 0))
  tail call void @abort()
  ret i1 true
...
case_branch:                                      ; preds = %entry
  ...
  %to_free = bitcast { i32, i64, i64 }* %0 to i8*
  call void @free(i8* %to_free)
  %apply_result = tail call i1 @free_multilist({ i32, i64, i64 }* %tmp3)
  ret i1 %apply_result

case_branch4:                                     ; preds = %entry
  %arg_ptr5 = getelementptr inbounds { i32, i64, i64 }, { i32, i64, i64 }* %0, i32 0, i32 1
  %arg_val6 = load i64, i64* %arg_ptr5, align 4
  %tmp7 = trunc i64 %arg_val6 to i1
  %arg_ptr8 = getelementptr inbounds { i32, i64, i64 }, { i32, i64, i64 }* %0, i32 0, i32 2
  %arg_val9 = load i64, i64* %arg_ptr8, align 4
  %tmp10 = inttoptr i64 %arg_val9 to { i32, i64, i64 }*
  %to_free11 = bitcast { i32, i64, i64 }* %0 to i8*
  call void @free(i8* %to_free11)
  %apply_result12 = tail call i1 @free_multilist({ i32, i64, i64 }* %tmp10)
  ret i1 %apply_result12

case_branch13:                                    ; preds = %entry
  %to_free14 = bitcast { i32, i64, i64 }* %0 to i8*
  call void @free(i8* %to_free14)
  ret i1 false
}

define { i32, i64, i64 }* @dup_multilist({ i32, i64, i64 }* %0) {
entry:
  %tag_ptr = getelementptr inbounds { i32, i64, i64 }, { i32, i64, i64 }* %0, i32 0, i32 0
  %tag_val = load i32, i32* %tag_ptr, align 4
  switch i32 %tag_val, label %default [
    i32 0, label %case_branch
    i32 1, label %case_branch8
    i32 2, label %case_branch23
  ]

default:                                          ; preds = %entry
  %tmp27 = call i32 (i8*, ...) @printf(i8* getelementptr inbounds ([38 x i8], [38 x i8]* @err_msg.22, i32 0, i32 0))
  tail call void @abort()
  ret { i32, i64, i64 }* inttoptr (i64 -1 to { i32, i64, i64 }*)
...
case_branch:                                      ; preds = %entry
  ...
  %elem_ptr = getelementptr inbounds { i32, i64, i64 }, { i32, i64, i64 }* %struct, i32 0, i32 1
  store i64 %tmp5, i64* %elem_ptr, align 4
  %apply_result = tail call { i32, i64, i64 }* @dup_multilist({ i32, i64, i64 }* %tmp3)
  %tmp6 = ptrtoint { i32, i64, i64 }* %apply_result to i64
  %elem_ptr7 = getelementptr inbounds { i32, i64, i64 }, { i32, i64, i64 }* %struct, i32 0, i32 2
  store i64 %tmp6, i64* %elem_ptr7, align 4
  ret { i32, i64, i64 }* %struct
...
case_branch8:                                     ; preds = %entry
  %arg_ptr9 = getelementptr inbounds { i32, i64, i64 }, { i32, i64, i64 }* %0, i32 0, i32 1
  %arg_val10 = load i64, i64* %arg_ptr9, align 4
  %tmp11 = trunc i64 %arg_val10 to i1
  %arg_ptr12 = getelementptr inbounds { i32, i64, i64 }, { i32, i64, i64 }* %0, i32 0, i32 2
  %arg_val13 = load i64, i64* %arg_ptr12, align 4
  %tmp14 = inttoptr i64 %arg_val13 to { i32, i64, i64 }*
  %malloccall15 = tail call i8* @malloc(i32 ptrtoint ({ i32, i64, i64 }* getelementptr ({ i32, i64, i64 }, { i32, i64, i64 }* null, i32 1) to i32))
  %struct16 = bitcast i8* %malloccall15 to { i32, i64, i64 }*
  %tag_ptr17 = getelementptr inbounds { i32, i64, i64 }, { i32, i64, i64 }* %struct16, i32 0, i32 0
  store i32 1, i32* %tag_ptr17, align 4
  %tmp18 = zext i1 %tmp11 to i64
  %elem_ptr19 = getelementptr inbounds { i32, i64, i64 }, { i32, i64, i64 }* %struct16, i32 0, i32 1
  store i64 %tmp18, i64* %elem_ptr19, align 4
  %apply_result20 = tail call { i32, i64, i64 }* @dup_multilist({ i32, i64, i64 }* %tmp14)
  %tmp21 = ptrtoint { i32, i64, i64 }* %apply_result20 to i64
  %elem_ptr22 = getelementptr inbounds { i32, i64, i64 }, { i32, i64, i64 }* %struct16, i32 0, i32 2
  store i64 %tmp21, i64* %elem_ptr22, align 4
  ret { i32, i64, i64 }* %struct16

case_branch23:                                    ; preds = %entry
  %malloccall24 = tail call i8* @malloc(i32 ptrtoint ({ i32, i64, i64 }* getelementptr ({ i32, i64, i64 }, { i32, i64, i64 }* null, i32 1) to i32))
  %struct25 = bitcast i8* %malloccall24 to { i32, i64, i64 }*
  %tag_ptr26 = getelementptr inbounds { i32, i64, i64 }, { i32, i64, i64 }* %struct25, i32 0, i32 0
  store i32 2, i32* %tag_ptr26, align 4
  ret { i32, i64, i64 }* %struct25
}

define { i32, i64, i64, i64 }* @_nil-bst() {
entry:
  %malloccall = tail call i8* @malloc(i32 ptrtoint ({ i32, i64, i64, i64 }* getelementptr ({ i32, i64, i64, i64 }, { i32, i64, i64, i64 }* null, i32 1) to i32))
  %struct = bitcast i8* %malloccall to { i32, i64, i64, i64 }*
  %tag_ptr = getelementptr inbounds { i32, i64, i64, i64 }, { i32, i64, i64, i64 }* %struct, i32 0, i32 0
  store i32 1, i32* %tag_ptr, align 4
  ret { i32, i64, i64, i64 }* %struct
}

define { i32, i64, i64, i64 }* @_buildbst(i32 %0, { i32, i64, i64, i64 }* %1, { i32, i64, i64, i64 }* %2) {
entry:
  %malloccall = tail call i8* @malloc(i32 ptrtoint ({ i32, i64, i64, i64 }* getelementptr ({ i32, i64, i64, i64 }, { i32, i64, i64, i64 }* null, i32 1) to i32))
  %struct = bitcast i8* %malloccall to { i32, i64, i64, i64 }*
  %tag_ptr = getelementptr inbounds { i32, i64, i64, i64 }, { i32, i64, i64, i64 }* %struct, i32 0, i32 0
  store i32 0, i32* %tag_ptr, align 4
  %tmp = zext i32 %0 to i64
  %elem_ptr = getelementptr inbounds { i32, i64, i64, i64 }, { i32, i64, i64, i64 }* %struct, i32 0, i32 1
  store i64 %tmp, i64* %elem_ptr, align 4
  %tmp1 = ptrtoint { i32, i64, i64, i64 }* %1 to i64
  %elem_ptr2 = getelementptr inbounds { i32, i64, i64, i64 }, { i32, i64, i64, i64 }* %struct, i32 0, i32 2
  store i64 %tmp1, i64* %elem_ptr2, align 4
  %tmp3 = ptrtoint { i32, i64, i64, i64 }* %2 to i64
  %elem_ptr4 = getelementptr inbounds { i32, i64, i64, i64 }, { i32, i64, i64, i64 }* %struct, i32 0, i32 3
  store i64 %tmp3, i64* %elem_ptr4, align 4
  ret { i32, i64, i64, i64 }* %struct
}

define i1 @free_bst({ i32, i64, i64, i64 }* %0) {
entry:
  %tag_ptr = getelementptr inbounds { i32, i64, i64, i64 }, { i32, i64, i64, i64 }* %0, i32 0, i32 0
  %tag_val = load i32, i32* %tag_ptr, align 4
  switch i32 %tag_val, label %default [
...
default:                                          ; preds = %entry
  %tmp10 = call i32 (i8*, ...) @printf(i8* getelementptr inbounds ([38 x i8], [38 x i8]* @err_msg.21, i32 0, i32 0))
  tail call void @abort()
  ret i1 true
...
case_branch:                                      ; preds = %entry
  ...
  %arg_val = load i64, i64* %arg_ptr, align 4
  %tmp = trunc i64 %arg_val to i32
  %arg_ptr1 = getelementptr inbounds { i32, i64, i64, i64 }, { i32, i64, i64, i64 }* %0, i32 0, i32 2
  %arg_val2 = load i64, i64* %arg_ptr1, align 4
  %tmp3 = inttoptr i64 %arg_val2 to { i32, i64, i64, i64 }*
  %arg_ptr4 = getelementptr inbounds { i32, i64, i64, i64 }, { i32, i64, i64, i64 }* %0, i32 0, i32 3
  %arg_val5 = load i64, i64* %arg_ptr4, align 4
  %tmp6 = inttoptr i64 %arg_val5 to { i32, i64, i64, i64 }*
  %to_free = bitcast { i32, i64, i64, i64 }* %0 to i8*
  call void @free(i8* %to_free)
  %apply_result = tail call i1 @free_bst({ i32, i64, i64, i64 }* %tmp3)
  %apply_result7 = tail call i1 @free_bst({ i32, i64, i64, i64 }* %tmp6)
  ret i1 %apply_result7

case_branch8:                                     ; preds = %entry
  %to_free9 = bitcast { i32, i64, i64, i64 }* %0 to i8*
  call void @free(i8* %to_free9)
  ret i1 false
}

define { i32, i64, i64, i64 }* @dup_bst({ i32, i64, i64, i64 }* %0) {
entry:
  %tag_ptr = getelementptr inbounds { i32, i64, i64, i64 }, { i32, i64, i64, i64 }* %0, i32 0, i32 0
  %tag_val = load i32, i32* %tag_ptr, align 4
  switch i32 %tag_val, label %default [
    i32 0, label %case_branch
    i32 1, label %case_branch14
  ]

default:                                          ; preds = %entry
  %tmp18 = call i32 (i8*, ...) @printf(i8* getelementptr inbounds ([38 x i8], [38 x i8]* @err_msg.20, i32 0, i32 0))
  tail call void @abort()
  ret { i32, i64, i64, i64 }* inttoptr (i64 -1 to { i32, i64, i64, i64 }*)
...
case_branch:                                      ; preds = %entry
  ...
  %malloccall = tail call i8* @malloc(i32 ptrtoint ({ i32, i64, i64, i64 }* getelementptr ({ i32, i64, i64, i64 }, { i32, i64, i64, i64 }* null, i32 1) to i32))
  %struct = bitcast i8* %malloccall to { i32, i64, i64, i64 }*
  %tag_ptr7 = getelementptr inbounds { i32, i64, i64, i64 }, { i32, i64, i64, i64 }* %struct, i32 0, i32 0
  store i32 0, i32* %tag_ptr7, align 4
  %tmp8 = zext i32 %tmp to i64
  %elem_ptr = getelementptr inbounds { i32, i64, i64, i64 }, { i32, i64, i64, i64 }* %struct, i32 0, i32 1
  store i64 %tmp8, i64* %elem_ptr, align 4
  %apply_result = tail call { i32, i64, i64, i64 }* @dup_bst({ i32, i64, i64, i64 }* %tmp3)
  %tmp9 = ptrtoint { i32, i64, i64, i64 }* %apply_result to i64
  %elem_ptr10 = getelementptr inbounds { i32, i64, i64, i64 }, { i32, i64, i64, i64 }* %struct, i32 0, i32 2
  store i64 %tmp9, i64* %elem_ptr10, align 4
  %apply_result11 = tail call { i32, i64, i64, i64 }* @dup_bst({ i32, i64, i64, i64 }* %tmp6)
  %tmp12 = ptrtoint { i32, i64, i64, i64 }* %apply_result11 to i64
  %elem_ptr13 = getelementptr inbounds { i32, i64, i64, i64 }, { i32, i64, i64, i64 }* %struct, i32 0, i32 3
  store i64 %tmp12, i64* %elem_ptr13, align 4
  ret { i32, i64, i64, i64 }* %struct

case_branch14:                                    ; preds = %entry
  %malloccall15 = tail call i8* @malloc(i32 ptrtoint ({ i32, i64, i64, i64 }* getelementptr ({ i32, i64, i64, i64 }, { i32, i64, i64, i64 }* null, i32 1) to i32))
  %struct16 = bitcast i8* %malloccall15 to { i32, i64, i64, i64 }*
  %tag_ptr17 = getelementptr inbounds { i32, i64, i64, i64 }, { i32, i64, i64, i64 }* %struct16, i32 0, i32 0
  store i32 1, i32* %tag_ptr17, align 4
  ret { i32, i64, i64, i64 }* %struct16
}

define { i32 }* @_down() {
entry:
  %malloccall = tail call i8* @malloc(i32 ptrtoint ({ i32 }* getelementptr ({ i32 }, { i32 }* null, i32 1) to i32))
  %struct = bitcast i8* %malloccall to { i32 }*
  %tag_ptr = getelementptr inbounds { i32 }, { i32 }* %struct, i32 0, i32 0
  store i32 3, i32* %tag_ptr, align 4
  ret { i32 }* %struct
}
...
define i1 @free_direction({ i32 }* %0) {
entry:
  %tag_ptr = getelementptr inbounds { i32 }, { i32 }* %0, i32 0, i32 0
  %tag_val = load i32, i32* %tag_ptr, align 4
  switch i32 %tag_val, label %default [
    i32 0, label %case_branch
    i32 1, label %case_branch1
    i32 2, label %case_branch3
    i32 3, label %case_branch5
  ]

default:                                          ; preds = %entry
  %tmp = call i32 (i8*, ...) @printf(i8* getelementptr inbounds ([38 x i8], [38 x i8]* @err_msg.19, i32 0, i32 0))
  tail call void @abort()
  ret i1 true
...
case_branch3:                                     ; preds = %entry
  %to_free4 = bitcast { i32 }* %0 to i8*
  call void @free(i8* %to_free4)
  ret i1 false

case_branch5:                                     ; preds = %entry
  %to_free6 = bitcast { i32 }* %0 to i8*
  call void @free(i8* %to_free6)
  ret i1 false
}

define { i32 }* @dup_direction({ i32 }* %0) {
entry:
  %tag_ptr = getelementptr inbounds { i32 }, { i32 }* %0, i32 0, i32 0
  %tag_val = load i32, i32* %tag_ptr, align 4
  switch i32 %tag_val, label %default [
    i32 0, label %case_branch
    i32 1, label %case_branch2
    i32 2, label %case_branch6
    i32 3, label %case_branch10
  ]

default:                                          ; preds = %entry
  %tmp = call i32 (i8*, ...) @printf(i8* getelementptr inbounds ([38 x i8], [38 x i8]* @err_msg.18, i32 0, i32 0))
  tail call void @abort()
  ret { i32 }* inttoptr (i64 -1 to { i32 }*)

case_branch:                                      ; preds = %entry
  %malloccall = tail call i8* @malloc(i32 ptrtoint ({ i32 }* getelementptr ({ i32 }, { i32 }* null, i32 1) to i32))
  %struct = bitcast i8* %malloccall to { i32 }*
  %tag_ptr1 = getelementptr inbounds { i32 }, { i32 }* %struct, i32 0, i32 0
  store i32 0, i32* %tag_ptr1, align 4
  ret { i32 }* %struct

case_branch2:                                     ; preds = %entry
  %malloccall3 = tail call i8* @malloc(i32 ptrtoint ({ i32 }* getelementptr ({ i32 }, { i32 }* null, i32 1) to i32))
  %struct4 = bitcast i8* %malloccall3 to { i32 }*
  %tag_ptr5 = getelementptr inbounds { i32 }, { i32 }* %struct4, i32 0, i32 0
  store i32 1, i32* %tag_ptr5, align 4
  ret { i32 }* %struct4

case_branch6:                                     ; preds = %entry
  %malloccall7 = tail call i8* @malloc(i32 ptrtoint ({ i32 }* getelementptr ({ i32 }, { i32 }* null, i32 1) to i32))
  %struct8 = bitcast i8* %malloccall7 to { i32 }*
  %tag_ptr9 = getelementptr inbounds { i32 }, { i32 }* %struct8, i32 0, i32 0
  store i32 2, i32* %tag_ptr9, align 4
  ret { i32 }* %struct8

case_branch10:                                    ; preds = %entry
  %malloccall11 = tail call i8* @malloc(i32 ptrtoint ({ i32 }* getelementptr ({ i32 }, { i32 }* null, i32 1) to i32))
  %struct12 = bitcast i8* %malloccall11 to { i32 }*
  %tag_ptr13 = getelementptr inbounds { i32 }, { i32 }* %struct12, i32 0, i32 0
  store i32 3, i32* %tag_ptr13, align 4
  ret { i32 }* %struct12
}

define { i32, i64, i64 }* @_nil-intlist() {
entry:
  %malloccall = tail call i8* @malloc(i32 ptrtoint ({ i32, i64, i64 }* getelementptr ({ i32, i64, i64 }, { i32, i64, i64 }* null, i32 1) to i32))
  %struct = bitcast i8* %malloccall to { i32, i64, i64 }*
  %tag_ptr = getelementptr inbounds { i32, i64, i64 }, { i32, i64, i64 }* %struct, i32 0, i32 0
...
define i1 @free_intlist({ i32, i64, i64 }* %0) {
entry:
  %tag_ptr = getelementptr inbounds { i32, i64, i64 }, { i32, i64, i64 }* %0, i32 0, i32 0
  %tag_val = load i32, i32* %tag_ptr, align 4
  switch i32 %tag_val, label %default [
...
default:                                          ; preds = %entry
  %tmp6 = call i32 (i8*, ...) @printf(i8* getelementptr inbounds ([38 x i8], [38 x i8]* @err_msg.17, i32 0, i32 0))
  tail call void @abort()
  ret i1 true
...
case_branch:                                      ; preds = %entry
  ...
  %to_free = bitcast { i32, i64, i64 }* %0 to i8*
  call void @free(i8* %to_free)
  %apply_result = tail call i1 @free_intlist({ i32, i64, i64 }* %tmp3)
  ret i1 %apply_result
...
define { i32, i64, i64 }* @dup_intlist({ i32, i64, i64 }* %0) {
entry:
  %tag_ptr = getelementptr inbounds { i32, i64, i64 }, { i32, i64, i64 }* %0, i32 0, i32 0
  %tag_val = load i32, i32* %tag_ptr, align 4
  switch i32 %tag_val, label %default [
...
case_branch:                                      ; preds = %entry
  ...
  %elem_ptr = getelementptr inbounds { i32, i64, i64 }, { i32, i64, i64 }* %struct, i32 0, i32 1
  store i64 %tmp5, i64* %elem_ptr, align 4
  %apply_result = tail call { i32, i64, i64 }* @dup_intlist({ i32, i64, i64 }* %tmp3)
  %tmp6 = ptrtoint { i32, i64, i64 }* %apply_result to i64
  %elem_ptr7 = getelementptr inbounds { i32, i64, i64 }, { i32, i64, i64 }* %struct, i32 0, i32 2
  store i64 %tmp6, i64* %elem_ptr7, align 4
  ret { i32, i64, i64 }* %struct
...
\subsection{Roger Burtonpatel} {
  ...
  \\\\
  1. While I enjoyed OCaml, I can confidently say I don't think having every 
  construct be let-bound is easy to read or particularly fun to program in. 
  I prefer the syntax of Standard ML and wish I could have used ML with the 
  tooling and features that OCaml has. Specifically, or-patterns, pattern 
  side conditions, and the advanced OCaml language server were all great - 
  just wish ML had modern FP language features. 
  \\\\
  2. Designing good error messages makes testing much easier. I spent 
  most of my effort in the type checker ensuring that failure had access 
  to as much information as possible from the context to give expressive 
  diagnostics. I even explored adding nuanced features, like colored warnings 
  and printing function type names, to make programming in Compost as easy 
  as possible given the restrictions. This made testing almost trivial: errors 
  were always diagnosed in the place they occured. In future compiler design, 
  I will continue to pay close attention to error messages; they saved me 
  many times the amount of time they took to write. 
  \\\\
  3. Having to add a bit of type information to declare a function isn't bad at
  all. Not having polymorphism really is. This was what I underestimated as far
  as detriments of no Hindly-Milner: redeclaring functions for every type is a
  pain. Because we didn't fully have HOFs with lambda, it wasn't so bad, but it
  still was annoying. Advice I'd give to a future team (and myself, if I were to
  redo this project) would be to ask to collaborate with someone else to build
  out full polymorphic type inference. I didn't have the time to do and debug it
  myself, and in hindsight I should have asked for help. Still, I'm proud of our
  group's work, and I look forward to seeing what all of us will build in the
  future.  
}

\subsection{Randy Dang} {
    My major takeaways from this project are that functional languages are powerful when implementing 
    compilers and that there is a lot of benefit to planning ahead. 
    \\\\
    Since language definitions and ultimately the abstract tree representations are naturally recursive, 
    having a language that is built for handling this recursive structure allowed for much more efficient 
    programming (last year for my Capstone project, my group implemented a Python transpiler in an 
    Object-Oriented manner, which was a less clean approach). Using existing parsing tools (such as Menhir)
    that already implemented complex parsing algorithms also simplified the compiler implementation 
    process, encouraging me not to ``re-invent the wheel'' when there already exists pretty sophisticated 
    methods for solving a problem.
    \\\\
    When we worked on a project as large as this one, we really benefitted from going into the project with 
    a plan in mind for how we were planning on organizing our passes, as that ultimately made development 
    more efficient in the long-run. On smaller scales, when we planned certain algorithms (implemented in 
    certain passes) ahead of time, we thought of potential issues that led to algorithm revisions before 
    even beginning to implement the pass. On the other hand, failing to consider certain issues ahead of 
    time sometimes led to a need to undo a substantial amount of work to restructure the algorithm. Thus, 
    the benefit of planning ahead really showed up in this project.
    \\\\
    My advice for future teams would be to consider as many potential issues as possible up front when 
    planning for the implementation rather than waiting for them to come up during the implementation process 
    in order to avoid spending a lot of time stuck at dead ends (although it is also important to plan for 
    the fact that there will inevitably be some issues only discovered during the implementation process).
    Hashing out the intermediate representations before implementing any compiler passes also makes it easy 
    to break up the compilation process into concrete, manageable pieces that can also be an effective way 
    to split up the work. 
    \\\\
    Also, use the OCaml language server to help with development. I was embarrassingly unaware of its existence 
    until two weeks before the end of the semester (and have coded without syntax highlighting up until 
    then).
}

\subsection{Jasper Geer} {
  I've written a few compilers at this point, but until now, I hadn't done any as a group.
  This was certainly a challenge but I think I've learned quite a bit from the experience.
  My first major takeaway from writing Compost is the importance of delegation.
  \\\\
  I think we settled on a pretty good method while writing Compost.
  Before any work started, we laid out the architecture of our compiler, creating a sequence of well-defined
  IRs and passes, each of which established one or two key invariants.
  This way, we could assign each group member a single pass or a couple of passes and they could do their
  work individually, trusting the author of the pass upstream to establish their invariants.
  \\\\
  This, of course, brings me to the importance of communication.
  In order for this strategy to work, each group member needed to be an expert in the particular pass and
  IR that they ``owned'' and at least be familar with the rest of the compiler.
  When passes or IRs had to be modified, it was extremely important to communicate to the rest of the group
  and make any directly affected individuals aware of precisely how this changed their job.
  Usually used to working on my own, this was a lesson I had to learn, especially when I had to add
  additional type information to a few IRs and add an entirely new one to make my affine type checking more
  streamlined.
  But, by the end, I think we reached a state where everyone was very knowledgeable in how their particular
  piece of compiler worked and why it had to work the way it did. I still have no idea \textit{how} our macros system
  works, but I don't need to.
  I know what it needs to do and how it affects me, and our tests do the rest.
  \\\\
  In terms of advice to future groups, if you feel comfortable, breaking your ``semant'' up into multiple
  stages lends itself to simpler code, makes bugs easier to find, and can help prevent the situation where
  your whole group is trying to add changes to the same file all at once.
  \\\\
  Also, don't try to do everything in codegen if you can help it.
  Intermediate representations that look progressively more like machine code (check out A-Normal form)
  can be very helpful when doing more complex code generation tasks.
  For example, our language supplies automatically generated deep copying functions.
  Rather than fumbling around with the not-so-great LLVM bindings, we generate the functions in an
  upstream intermediate representation and compile them in the same way we compile user-created code.
}

\subsection{Jackson Warhover} {
    A major takeaway I had was that initial planning and design can do wonders in streamlining the process
    of writing a complex project like a compiler. Focusing on the LRM in early stages and working through
    to make sure invariants were understood were absolutely critical steps.
    \\\\
    We had a good plan from the start for exactly how to implement IRs, which made creating them very simple. 
    It also made it very easy to add new IRs as they were necessary. I think a lack of design considerations
    in the early stages would have made implementing new IRs much harder.
    \\\\
    I also learned that communication of invariants is important. For invariants that are not inherently
    described by the structure of the types in the LRM, it is important for all members of a group to understand
    what the purpose of an IR is in order to reduce redundancy. Clear communication of ideas about what section
    is responsible for what means cleaner code in the end.
    \\\\
    In terms of testing, I learned just how critical the initial work on testing infrastructure can be. For us,
    a lot of this work was correctly put in early on, and refactors were made as necessary, which might have
    been time-consuming in the moment but equated to a significant reduction in workload for tests going
    forward.
    \\\\
    I also agree with Randy about how functional programing is perfect for writing a compiler. This might just
    be because I'm used to it at this point, but I really can't imagine implementing something nearly as clean
