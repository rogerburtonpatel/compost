...
\subsection{Feature 1: Memory Safety with Affine Types} {
  ...
  \textit{compile} time.
  To do this, we need to exactly when heap values become unreachable, and are thus fair game
  to dispose of.
...
\subsection{Feature 2: Partial Type Inference} {
  ...
  types.
  When evaluating a given expression, the type inference function is passed the
  \\\\
  ...
  a function application depends on the type of the called function, for example.
...
\subsection{Feature 3: ADTs and Top-Level Pattern Matching} {
  We support recursive abstract data types in a similar manner to ML and a
  minimally expressive pattern matching mechanism.
  \\\\
  Under the hood, datatype values are pointers to structs of 64-bit integers,
  the first field representing the value's constructor and the remaining fields
  the arguments the constructor is applied to.
  \\\\
...
\subsubsection{Tailcall Optimization} {
  ...
  passed a boolean telling it if the expression it is generating code for is in tail
  position.
  ...
  to mark it for tail call optimization.
}

\subsubsection{Higher Order Functions} {
  Function values in Compost are simply function pointers, which can be used in the same
  ways as any other value except they cannot be created inline.
