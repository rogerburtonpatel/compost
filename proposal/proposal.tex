...
Implementing Hindley-Milner Type Inference allows for users to define let expressions without explicitly stating the types of the bindings, avoiding all sorts of pain in defining and applying type constructors. The compiler would instead infer the types based on how it is used.
...
(define concat (xxs ys)
    (match xxs  ;; a pattern match. `xxs' is now considered out-of-scope
        [(cons x xs)    ;; a pattern. this deallocates `xxs''s top level `cons'
            ...
            (cons x (concat xs ys)) ;; `cons' is used to introduce a `list',
